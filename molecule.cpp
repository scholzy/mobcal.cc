...
Molecule rotate(Molecule molecule, double theta, double phi, double gamma)
{
	...
	for (unsigned i = 0; i < molecule.size(); ++i) {
		...
		double distance_xy = std::sqrt(atom.x * atom.x +
					       atom.y * atom.y);
	...
	for (unsigned i = 0; i < molecule.size(); ++i) {
		...
		double distance_zy = std::sqrt(atom.z * atom.z +
					       new_atom.y * new_atom.y);
		double n_phi = phi;
		if (distance_zy != 0) {
	...
	for (unsigned i = 0; i < molecule.size(); ++i) {
		Atom new_atom = new_molecule[i];
		double distance_xy = std::sqrt(new_atom.x * new_atom.x +
					       new_atom.y * new_atom.y);
...
double align_x(Molecule* molecule)
{
	...
	for (unsigned i = 0; i < molecule->size(); ++i) {
		Atom atom = (*molecule)[i];
		double distance = std::sqrt(atom.x * atom.x +
					    atom.y * atom.y +
					    atom.z * atom.z);
	...
	double distance_zy = std::sqrt(atom_max.z * atom_max.z +
				       atom_max.y * atom_max.y);
	...
	double distance_xy = std::sqrt(new_max.x * new_max.x +
				       new_max.y * new_max.y);
